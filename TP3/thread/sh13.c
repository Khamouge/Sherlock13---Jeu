...
void sendMessageToServer(char *ipAddress, int portno, char *mess)
{
	...
	printf("Tentative de connexion au serveur %s:%d avec le message : %s\n", ipAddress, portno, mess);
    bzero((char *) &serv_addr, sizeof(serv_addr));
    serv_addr.sin_family = AF_INET;
...
int main(int argc, char ** argv)
{
   ...
   /*La fonction pthread_create renvoie également une valeur, positive si tout se passe bien et négative sinon
    pthread_create(&tid, NULL, ptrVersFonction, NULL);*/
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				if ((mx<200) && (my<50) && (connectEnabled==1))
				{
					...
					sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer); // Ajout
		...
		switch (gbuffer[0])
		{
			...
			case 'I':
				// RAJOUTER DU CODE ICI
				/*Indique le nombre de joueurs connecté (réponse du serveur avant la connexion du joueur actuel)*/
				sscanf(gbuffer, "I %d", &gId); // Ajout
				printf("gId : %d\n",gId); // Permet de vérifier que le serveur a bien reçu la commande - Ajout aussi (On oublie pas le \n)

				break;
			// Message 'L' : le joueur recoit la liste des joueurs
			case 'L':
				// RAJOUTER DU CODE ICI
				/*Affiche le nom des joueurs connectés suivis de “-” s’il en manque*/
				sscanf(gbuffer, "L %s %s %s %s", gNames[0], gNames[1], gNames[2], gNames[3]); // Ajout
				printf("J1 : %s ; J2 : %s ; J3 : %s ; J4 : %s\n", gNames[0], gNames[1], gNames[2], gNames[3]); // Ajout aussi (On oublie pas le \n)

				break;
			// Message 'D' : le joueur recoit ses trois cartes
			case 'D':
				// RAJOUTER DU CODE ICI
				/*Distribue 3 cartes à un joueur - Arguments : numéro des cartes*/
				sscanf(gbuffer, "D %d %d %d", &b[0], &b[1], &b[2]); // Ajout
				printf("Carte 1 : %d ; Carte 2 ; %d ; Carte 3 : %d\n",b[0],b[1],b[2]); // Ajout aussi (On oublie pas le \n)
			...
			case 'M':
				// RAJOUTER DU CODE ICI
				/*Le joueur reçoit un numéro correspondant au numéro du joueur courant, ceci permettra
				d'afficher ou non le bouton go avec la variable goEnabled*/
				int numJoueur;
				sscanf(gbuffer, "M %d", &numJoueur); // Ajout
				printf("Numero du Joueur : %d\n",numJoueur + 1); // Ajout aussi (On oublie pas le \n)
				if (numJoueur==gId)
					goEnabled=1;
				else
					goEnabled=0;
			...
			case 'V':
				// RAJOUTER DU CODE ICI
				/*Le joueur recoit une valeur de tableCartes
